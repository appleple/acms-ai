...
### ブロックエディター連携
ブロックエディター右上の「AIアシスタント」ボタンから、選択中のテキストを文脈にしてチャットできます。
テキストを選択していない場合は、カーソルがあるテキストブロック全体が対象です。ユニットとカスタムフィールドの
どちらのブロックエディターでも利用できます。

AIの応答は「挿入」ボタンで対象範囲へテキストとして反映され、HTMLとしては実行されません。反映は通常の
ブロックエディター操作と同じく保存対象となり、Undo/Redoも利用できます。ドロワーを開いた後に本文や選択範囲を
変更した場合は、別の位置を誤って上書きしないよう挿入を中止します。その場合は対象を選び直して
AIアシスタントを開き直してください。画像・ファイルなど、テキストを持たないブロックは対象外です。
